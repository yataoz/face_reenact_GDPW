...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda'):
        ...
        #self._setup_renderer(self.cfg.model)   # commented out by Yatao
    ...
    def encode(self, images, use_detail=True):
        ...
        # Yatao: add all zero eye pose
        batch_size = images.shape[0]
        codedict['eye_pose'] = torch.zeros([1, 6], dtype=codedict['pose'].dtype, device=codedict['pose'].device, requires_grad=False).expand(batch_size, -1)
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        ## decode
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'], eye_pose_params=codedict['eye_pose'])
